...
class Stem(nn.Module):
    """Stem layer
     Image size adapt to Primary Caps:112*112 -> 28*28"""
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride):
        ...
        self.conv = nn.Conv2d(in_channels=in_channels,
                              ...
                              stride=stride)
        # self.ln = nn.LayerNorm(normalized_shape=out_channels, data_format="channels_first")
...
class CapsNet(nn.Module):
    ...
    def __init__(self):
        ...
        # Conv2d layer
        self.conv = nn.Conv2d(3, 256, (9, 9))
        self.relu = nn.ReLU(inplace=True)
        self.stem = Stem(in_channels=1, out_channels=3, kernel_size=4, stride=4)
        # Primary capsule
...
# def main():
#     tmp = torch.randn(1, 1, 112, 112)
#     model = CapsNet()
#     out_tmp = model(tmp)
#     print(out_tmp.shape)

# torch.Size([1, 10, 16])
# torch.Size([1, 10])         logit = torch.norm()  || a^2 + b^2 + ... + z^2 ||
