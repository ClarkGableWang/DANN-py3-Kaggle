...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_rate=0., layer_scale_init_value=1e-6):
        super().__init__()
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=(7, 7), padding=(3, 3), groups=dim)  # depthwise conv
...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans: int = 1, num_classes: int = 1000, depths: list = None,
        ...
        stem = nn.Sequential(nn.Conv2d(in_chans, dims[0], kernel_size=(4, 4), stride=(4, 4)),
        ...
        self.class_classifier.add_module('c_fc1', nn.Linear(dims[-1], num_classes))
        ...
        # self.class_classifier.add_module('c_fc3', nn.Linear(128, num_classes))
        self.class_classifier.add_module('c_softmax', nn.LogSoftmax(dim=1))

        self.domain_classifier = nn.Sequential()
        # self.domain_classifier.add_module('c_drop3', nn.Dropout(p=0.2))
        self.domain_classifier.add_module('d_fc1', nn.Linear(dims[-1], 2))
    ...
    def forward(self, x: torch.Tensor, alpha):
        x = self.forward_features(x)  # (N, 768)
        # x = self.head(x)
...
# torch.Size([1, 768, 3, 3])      (through x.mean([-2, -1]))
# torch.Size([1, 10])              through  self.head(dims[-1], num_class)
